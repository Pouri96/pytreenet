...
def contract_bra_to_ket_and_blocks_ignore_one_leg(bra_tensor: np.ndarray,
    ...
    for neighbour_id in ket_node.neighbouring_nodes():
        if neighbour_id != next_node_id:
            ...
            legs_bra.append(ket_node.neighbour_index(neighbour_id))
    legs_block.append(1)
    num_neighbours = bra_node.nneighbours()
...
def contract_leafs_Lindblad(node_id1, node_id2, state):
    tensor = np.tensordot(state.tensors[node_id1], state.tensors[node_id2],
                        axes=(state.nodes[node_id1].open_legs[0], state.nodes[node_id1].open_legs[0]))
    return tensor
...
def contract_subtrees_using_dictionary_Lindblad(node_id1: str,
    ...
    ketblock_tensor = contract_all_but_one_neighbour_block_to_ket(ket_tensor,
                                                                ...
                                                                next_node_id,
                                                                dictionary)
    bra_node, bra_tensor = state[node_id2]
    return contract_bra_to_ket_and_blocks_ignore_one_leg(bra_tensor,
                                                            ketblock_tensor,
                                                            bra_node,
                                                            ket_node,
...
def contract_any_Linblad(node_id1, node_id2, parent_id, state, dictionary):
    node = state.nodes[node_id1]
    if node.is_leaf():
        return contract_leafs_Lindblad(node_id1, node_id2, state)
...
def contract_node_with_environment_Lindblad(root_id, state, dictionary):

    ket_node, ket_tensor = state[root_id]
    ketblock_tensor = contract_all_neighbour_blocks_to_ket_Lindblad(ket_tensor,
                                                                    ket_node,
                                                                dictionary)
    perm = list(range(ketblock_tensor.ndim)) 
    perm.append(perm.pop(1))
    ket_neigh_block = ketblock_tensor.transpose(perm)
    i, j = root_id.replace('Site(', '').replace(')', '').split(',')
    bra_id = f"Node({i},{j})"
    bra_node, bra_tensor = state[bra_id]
...
def contract_ttn_Lindblad(vectorized_pho):    
    ...
    for i, node_id1 in enumerate(computation_order[len(state.nodes) // 2:]):  # The last one is the root node
        ...
        node = state.nodes[node_id1]
        parent_id = node.parent
        block = contract_any_Linblad(node_id1, node_id2 ,parent_id,
                                state,
                                dictionary)
    ...
    return complex(contract_node_with_environment_Lindblad(state.root_id,
                                                           state,
                                                           dictionary))        