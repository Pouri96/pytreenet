...
from pytreenet.contractions.contraction_util import contract_neighbour_block_to_ket

def contract_all_neighbour_blocks_to_ket_Lindblad(ket_tensor: np.ndarray,
                                                  ket_node: Node,
                                                  partial_tree_cache: PartialTreeCachDict) -> np.ndarray:
    """
    Contract all neighbour blocks to the ket tensor.
    ...
    Returns:
        ...
        """
    result_tensor = ket_tensor
    ket_children = ket_node.children[1:]

    for neighbour_id in ket_children:
        # A the neighbours are the same as the leg order, the tensor_leg_to_neighbour
        # is always 0.
        result_tensor = contract_neighbour_block_to_ket(result_tensor,
                                                        ket_node,
