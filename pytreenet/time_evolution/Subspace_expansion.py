...
def enlarge_ttn1_bond_with_ttn2_Lanczos(ttn1, ttn2, tol, Lanczos_threshold , k_fraction, validity_fraction, increase_fraction, max_iter):
   ...
   for i,node_id in enumerate(path_main[:-1]): 
        ...
        if ttn1_copy.is_in_canonical_form():
            if len(path_next[i]) > 1:
                ...
                ttn1_copy = adjust_ttn1_structure_to_ttn2(ttn1_copy,ttn1_structure)
        if ttn2.is_in_canonical_form():
            if len(path_next[i]) > 1:
                ttn2.orthogonality_center_id = path_next[i][0]

                ttn2_structure = deepcopy(ttn2)
                ttn2.move_orthogonalization_center(path_next[i][-1])
                ttn2 = adjust_ttn1_structure_to_ttn2(ttn2,ttn2_structure)
   ...
   ttn3 = adjust_ttn1_structure_to_ttn2(ttn3,ttn1_structure)
...
def pad_zero_ttn1_with_ttn2(ttn1, ttn2):
    ttn1 = adjust_ttn1_structure_to_ttn2(ttn1,ttn2)
    ttn3 = deepcopy(ttn1)
